...
cv::Mat TacticalPipeline::process_frame(const Detections& players,
    ...
    if (valid) {
        team1_pts = homography_.transform_to_pitch(team1_dets.centers());
        team2_pts = homography_.transform_to_pitch(team2_dets.centers());
        ball_pts = homography_.transform_to_pitch(balls.centers());
        ref_pts = homography_.transform_to_pitch(referees.centers());
    }
